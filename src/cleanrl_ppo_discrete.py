"""
This is a copy of the CleanRL ppo implementation that we just modify slightly so we
can load in our custom environments and train optimal policies on them.
We store these optimal policy checkpoints in the data/policies.

The purpose of this file is to be run standalone, and produce optimal policies for a given environment
that can then be loaded into the data generation process.
"""
...
#KEY: import our custom environments
import envs
...
#         floats = [float(x) for x in cleaned_string.split(',')]
#         return tuple(floats)
#     except ValueError:
...
def parse_args():
    ...
    #LINAS
    parser.add_argument("--policy-name",default="PPO",choices=["PPO"],type=str,
        help="name/class of the policy that interacts with the env")
    # parser.add_argument("--haider-env",type=bool,default=False,
    #     help="if the type is haider, then we use the haider env")
    ...
    #LINAS: change num-steps to max-steps-per-episode
...
def make_env_cleanrl(env_id, seed, idx, capture_video, run_name, options=None):
    def thunk():
...
if __name__ == "__main__":
    ...
    next_obs = torch.Tensor(envs.reset(seed=args.seed)[0]).to(device)  # don't retrieve reset infos
    next_done = torch.zeros(args.num_envs).to(device)
    #num_updates is the same as num_episodes
        ...
        for step in range(0, args.num_steps):
            ...
            for item in info.get("final_info", []):
                if item is not None and "episode" in item.keys():
                    ...
                    print(f"episode: {episode_num}, episodic_return={item['episode']['r']}, total timesteps: {global_step}")
                    episode_num += 1
    ...
    if args.env_id in ["IMANOCartpoleEnv-v0", "IMANSCartpoleEnv-v0", "TimeSeriesEnv-v0"]:
        model_path = os.path.join(
            ...
            f"{args.total_timesteps}_timesteps",
            generate_hash(),
            "model.pth")
    ...
    save_path = os.path.join(parent_dir, model_path)
